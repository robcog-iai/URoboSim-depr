...
void ARRobot::BeginPlay()
{
	...
	Handler = new FROSBridgeHandler(TEXT("192.168.1.11"), 9090);
...
void ARRobot::Tick(float DeltaTime)
{
	...
	for (auto &JointElement : JointComponents) {
		...
		FString ParentCompName = JointElement.Value->ComponentName1.ComponentName.ToString();
		FString ChildCompName = JointElement.Value->ComponentName2.ComponentName.ToString();
		...
		{
			FString LJointName = JointElement.Value->GetName();

			FVector JointVelocityChild = ChildComponent->GetPhysicsAngularVelocity();
			FVector JointVelocityParent = ParentComponent->GetPhysicsAngularVelocity();
			FVector JointVelocity = JointVelocityChild - JointVelocityParent;
			UE_LOG(LogTemp, Log, TEXT("Joint Velocity [%s]"), *JointVelocity.ToString());
			...
			FQuat InitialRotationRel = OriginRotations[LJointName];
			FQuat QRel = CurrentRotationRel * InitialRotationRel.Inverse();
			FVector Axis; float Angle;
			QRel.ToAxisAndAngle(Axis, Angle);

			// Get Axis
			auto MotionSwing1 = JointElement.Value->ConstraintInstance.GetAngularSwing1Motion();
			auto MotionSwing2 = JointElement.Value->ConstraintInstance.GetAngularSwing2Motion();
			auto MotionTwist = JointElement.Value->ConstraintInstance.GetAngularTwistMotion();
			...
			{
				// not a hinged joint
				UE_LOG(LogTemp, Error, TEXT("Joint [%s] is not a hinged joint!"), *JointElement.Value->GetName());
			}
			else if ((rotationX && rotationY) || (rotationX && rotationZ) || (rotationY && rotationZ))
			{
				// not a hinged joint
				UE_LOG(LogTemp, Error, TEXT("Joint [%s] is not a hinged joint with DOF=1"), *JointElement.Value->GetName());
			}
			else
			{
				// a hinged joint
				...
				UE_LOG(LogTemp, Log, TEXT("Joint [%s] Link [%s] -> [%s] Current Axis: [%s] Angle: [%.3f] \n InitialRel: [%s] CurrentRel: [%s] "),
					*JointElement.Value->GetName(), *ParentComponent->GetName(), *ChildComponent->GetName(),
			...
			TArray<FString> names = { TEXT(JointName), TEXT(JointName), TEXT(JointName) };
			TArray<double > position, velocity, effort;
			position = { InitialRotationRel.Rotator().Pitch, InitialRotationRel.Rotator().Yaw, InitialRotationRel.Rotator().Roll };
			velocity = { JointVelocity.X, JointVelocity.Y, JointVelocity.Z };									
			effort = { 0.0f, 0.0f, 0.0f }; //not sending any effort messages for now
